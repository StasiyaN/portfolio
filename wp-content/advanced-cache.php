...
// WP-Optimize advanced-cache.php (written by version: 3.5.0) (homeurl: https://nyzana-dev.local/) (abspath: C:\Users\asian\Local Sites\nyzana-dev\app\public/) (do not change this line, it is used for correctness checks)
...
$possible_plugin_locations = array(
	...
	'C:\Users\asian\Local Sites\nyzana-dev\app\public\wp-content\plugins\wp-optimize\cache',
);
...
if (false === $plugin_location) {
	...
	if (strcasecmp('https://nyzana-dev.local/', $protocol . '://' . $host . $request_uri) === 0) {
...
if (false !== $plugin_location) {
	...
	if (!defined('WPO_CACHE_EXT_DIR')) define('WPO_CACHE_EXT_DIR', 'C:\Users\asian\Local Sites\nyzana-dev\app\public\wp-content\plugins\wp-optimize\cache/extensions');
}

if (!@file_exists(WPO_CACHE_CONFIG_DIR . '/config-nyzana-dev.local.php')) { return; }

$GLOBALS['wpo_cache_config'] = @json_decode(file_get_contents(WPO_CACHE_CONFIG_DIR . '/config-nyzana-dev.local.php'), true);

if (empty($GLOBALS['wpo_cache_config'])) {
	include_once(WPO_CACHE_CONFIG_DIR . '/config-nyzana-dev.local.php');
}
